$(document).ready(function() {
  ...
  d3.json("data.json", function(error, data) {
    var diffDiary = 0;
    var diffWeekly = 0;
    var MaxValue = d3.max(data, function( d ) { return d.MN; });

    data.reverse();

    //xAxis.tickValues([0, (MaxValue / 4), (MaxValue / 2), (MaxValue / 1.3), MaxValue]);
    xPercent.domain([0, d3.max("1")]);
    x.domain([0, d3.max(data, function( d ) { return d.MN; })]);
    ...
    bar.selectAll(".j").transition().ease("quad-out").duration(2000).delay(0).attr("width", function(d) { return x(d.MN); }).each('end', function() {
      activeTooltip = false;
      //activeDblClick = false;
      //tapped=false;
    });
