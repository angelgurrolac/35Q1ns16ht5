$(document).ready(function() {
  ...
  var margin = {top: 20, right: 30, bottom: 20, left: 30},
    width = $(".transSecWeek").width(),
    height = $('.transSecToday').height();
  ...
  var svgCont = d3.select("#WeekTransactionsSecond").append("svg")
      .attr({
        "width": width,
  ...
  d3.json("BarCharHorWeekTranPerSecond/data.json", function(error, data) {
    var diffDiary = 0;
    var diffWeekly = 0;
    var MaxValue = d3.max(data, function( d ) { return d.transactions; });

    data.reverse();

    //xAxis.tickValues([0, (MaxValue / 4), (MaxValue / 2), (MaxValue / 1.3), MaxValue]);
    xPercent.domain([0, d3.max("1")]);
    x.domain([0, d3.max(data, function( d ) { return d.transactions; })]);
        ...
        .on('click', function(d, i, e) {
          ...
          svg.selectAll( '.too' ).remove();
            ...
            tooltip.append("text")
              ...
              .attr("x", 8)
              .attr("y", 18)
              .text(formatNumber(d.transactions) /*+ " → "*/);
            ...
            //console.log(d.avgDiary);
            diffDiary = (d.transactions / d.avgDiary) - 1;
            diffWeekly = (d.transactions / d.avgWeekly) - 1;
    ...
    bar.selectAll(".j").transition().ease("quad-out").duration(2000).delay(0).attr("width", function(d) { return x(d.transactions); }).each('end', function() {
      activeTooltip = false;
      //activeDblClick = false;
      //tapped=false;
    });
