...
function RadialWorking() {
  ...
  */
  ...
  d3.json("../js/DataAPIResponse.json", function(data){
    for (var i = 0; i <= data.kpis['sstob.atms'].data[0].values[0].data.length - 1; i++) {
      avgPercent = avgPercent + data.kpis['sstob.atms'].data[0].values[0].data[i][1];
    }
    avgPercent = avgPercent / data.kpis['sstob.atms'].data[0].values[0].data.length;
    data = data.kpis['sstob.atms'].data[0].values[0].data[0];

    percent = (data[1]/data[3])/1;//percent is equals to a total of ATM's that are working
    ...
    avgPercent = (avgPercent/data[3])/1;//avgPercent gives the average
...
/*$(function(){
  $('#radialChart[onload]').trigger('onload');
  alert('ja');
});*/
