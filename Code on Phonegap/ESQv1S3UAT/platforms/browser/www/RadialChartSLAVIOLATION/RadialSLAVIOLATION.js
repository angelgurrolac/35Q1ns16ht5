//$(document).ready(function() {
  ...
  */
  ...
  var width = 177, //Get in the variable width the container width
  ...
  d3.json("../js/DataAPIResponse.json", function(data){
    for (var i = 0; i <= data.kpis['sstob.sla'].data[0].values[0].data.length - 1; i++) {
      avgPercent = avgPercent + data.kpis['sstob.sla'].data[0].values[0].data[i][1];
    }
    avgPercent = avgPercent / data.kpis['sstob.sla'].data[0].values[0].data.length;

    data = data.kpis['sstob.sla'].data[0].values[0].data[0];
    percent = (data[1]/data[2]);//percent is equals to a total of ATM's that are working
    ...
    avgPercent = (avgPercent/data[2]);//avgPercent gives the average
