$(document).ready(function() {
  ...
  var total;
  function ಠ_ಠ() {
    d3.json("AreaChartTodayRESPONSETIME/data.json", function(datas) {
      drawLineChart(    'TodayResponseT',    datas.lineChart );
    });
  }
  ...
  function drawLineChart( elementId, data ) {
    ...
    total = d3.max(data, function( d ) { return d.responsetime; });
    ...
    function drawCircle( datum, index ) {
      circleContainer.datum( datum )
                    ...
                    .attr(
                      'cy',
                      function( d ) {
                        if (d.responsetime === 0 || d.responsetime === null) {
    ...
    function showCircleDetail( data ) {
      var details = circleContainer.append( 'g' )
                        ...
                        .attr(
                          ...
                          function() {
                            ...
                            result += ', ';
                            result += y( data.responsetime ) - detailHeight - detailMargin;
                            result += ')';
      ...
      var DiffPercentage = text.append( 'tspan' )
          ...
          .attr( 'x', detailWidth / 1.6 )
          .attr( 'y', detailHeight / 4 * 2.7 )
          .attr( 'text-anchor', 'start' );

      difference = (data.responsetime / data.avg) - 1;

      if (difference < 0) {
        triangle.attr("d", "M48 32 L53 36 L58 32 Z")
          .style("fill", "#3A8686");
        difference = Math.abs(difference);
        DiffPercentage.style("fill", "#3A8686");
      }
      else if (difference > 0) {
        triangle.attr("d", "M48 35 L53 32 L58 35 Z")
          .style("fill", "#E3509D");
      ...
      text.append( 'tspan' )
          ...
          .text(function() {
            return formatNumbers(data.responsetime) ;
          });
    }
